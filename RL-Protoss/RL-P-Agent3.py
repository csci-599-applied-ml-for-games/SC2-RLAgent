...
class QLearningTable:
    def __init__(self, actions, learning_rate=0.1, gamma=0.9):
        ...
        if os.path.isfile('./q-table3.csv'):
            ...
            self.q_table = pd.read_csv('./q-table3.csv', header=0, index_col=0)
        else:
    ...
    def decide(self, observation, explore=0.8):
    ...
    def learn(self, s, a, r, s_):
        ...
        cur_q = self.q_table.loc[s, a]
        fut_q = r + self.gamma * self.q_table.loc[s_, :].max()

        self.q_table.loc[s, a] += self.learning_rate * (fut_q - cur_q)
...
class ProtossAgent(Agent):
    ...
    def step(self, obs):
        ...
        if self.previous_action is not None:
            self.q_table.learn(self.previous_state, self.previous_action, reward, state)
...
def main(unused_argv):
    ...
    agent1.q_table.q_table.to_csv('q-table3.csv')
