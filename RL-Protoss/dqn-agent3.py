...
class DQN:
    def __init__(self, input_shape, actions, buffer_size, gamma):
        self.actions = actions
        if os.path.isdir('./dqn_model_3'):
            print('Using existing model')
            self.decision_net = tf.keras.models.load_model('./dqn_model_3') # Sequential network used for making decisions
            ...
            self.learning_net = tf.keras.models.load_model('./dqn_model_3') # Sequential network used for learning
            ...
            self.decision_net = Sequential()
            self.decision_net.add(Conv2D(32, kernel_size=(3, 3), activation='relu', input_shape=input_shape, padding='same'))
            self.decision_net.add(MaxPooling2D(pool_size=(2, 2)))
            self.decision_net.add(BatchNormalization())
            #model.add(Dropout(0.2))
            self.decision_net.add(Conv2D(64, kernel_size=(3, 3), activation='relu', padding='same'))
            self.decision_net.add(MaxPooling2D(pool_size=(2, 2)))
            self.decision_net.add(BatchNormalization())
            #model.add(Dropout(0.2))
            self.decision_net.add(Conv2D(128, kernel_size=(3, 3), activation='relu', padding='same'))
            self.decision_net.add(MaxPooling2D(pool_size=(2, 2)))
            self.decision_net.add(BatchNormalization())
            self.decision_net.add(Flatten())
            self.decision_net.add(Dense(512, activation='relu'))
            self.decision_net.add(Dense(256, activation='relu'))
            #model.add(Dropout(0.4))
            self.decision_net.add(Dense(len(actions), activation=None))
            self.decision_net.compile(loss='huber_loss', optimizer='RMSprop', metrics=['MeanSquaredError'])
            print(self.decision_net.summary())
            self.learning_net = Sequential()
            self.learning_net.add(Conv2D(32, kernel_size=(3, 3), activation='relu', input_shape=input_shape, padding='same'))
            self.learning_net.add(MaxPooling2D(pool_size=(2, 2)))
            self.learning_net.add(BatchNormalization())
            #model.add(Dropout(0.2))
            self.learning_net.add(Conv2D(64, kernel_size=(3, 3), activation='relu', padding='same'))
            self.learning_net.add(MaxPooling2D(pool_size=(2, 2)))
            self.learning_net.add(BatchNormalization())
            #model.add(Dropout(0.2))
            self.learning_net.add(Conv2D(128, kernel_size=(3, 3), activation='relu', padding='same'))
            self.learning_net.add(MaxPooling2D(pool_size=(2, 2)))
            self.learning_net.add(BatchNormalization())
            self.learning_net.add(Flatten())
            self.learning_net.add(Dense(512, activation='relu'))
            self.learning_net.add(Dense(256, activation='relu'))
            #model.add(Dropout(0.4))
            self.learning_net.add(Dense(len(actions), activation=None))
            self.learning_net.compile(loss='huber_loss', optimizer='RMSprop', metrics=['MeanSquaredError'])
            #self.decision_net.summary()
    ...
    def learn(self):
        indexes = np.random.choice(len(self.buffer), 64, replace=False)
        ...
        '''
        for i in range(len(s_)):
            sq = self.learning_net.predict(np.array([s[i]]))
            s_q = self.learning_net.predict(np.array([s_[i]]))
            tmp = np.add(sq, 0.9*np.subtract(s_q, sq))
            tmp[0][self.actions.index(a[i])] += r[i]
            target.append(tmp) 

        target = np.array(target)
        self.learning_net.fit(s, target, batch_size=64, epochs=1)
        self.counter += 1
        if self.counter >= 30:
...
class ProtossAgent(Agent):
    def __init__(self):
        ...
        self.dqn = DQN((64, 64, 3), self.actions, 5000, 0.1)
    ...
    def get_image(self, obs):
        game_map = np.full((64, 64, 3), 255, np.uint8) #Use white as background color
...
def main(unused_argv):
    ...
    if mode == 'No':
        ...
        try:
            with sc2_env.SC2Env(map_name = 'Simple64', players=[sc2_env.Agent(sc2_env.Race.protoss), sc2_env.Bot(sc2_env.Race.protoss, sc2_env.Difficulty.medium)],
                                agent_interface_format=features.AgentInterfaceFormat(action_space=actions.ActionSpace.RAW, use_raw_units=True, raw_resolution=64),
            ...
            pass 
    agent1.dqn.learning_net.save('./dqn_model_3/')
