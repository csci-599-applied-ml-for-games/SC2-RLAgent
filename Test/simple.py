# -*- coding: utf-8 -*-
"""
Created on Thu Mar 19 09:51:18 2020
...
from sc2.constants import NEXUS,PYLON,ASSIMILATOR,GATEWAY,FORGE,CYBERNETICSCORE,ROBOTICSFACILITY,TWILIGHTCOUNCIL,PROBE,ZEALOT,STALKER,IMMORTAL
import random
...
class SentdeBot(sc2.BotAI):
    ...
    async def offensive_force_buildings(self):
        ...
        if self.units(PYLON).ready.exists:
            ...
            # 建造更多BG
                ...
                if self.can_afford(GATEWAY) and not self.already_pending(GATEWAY) and self.units(GATEWAY).amount < 8:
                    await self.build(GATEWAY, near=pylon)
            # 这个VS放的早啊
            if self.units(CYBERNETICSCORE).ready.exists:
                if len(self.units(ROBOTICSFACILITY)) < ((self.iteration / self.ITERATIONS_PER_MINUTE) / 2):
                    if self.can_afford(ROBOTICSFACILITY) and not self.already_pending(ROBOTICSFACILITY) and self.units(ROBOTICSFACILITY).amount < 2:
    ...
    async def build_offensive_force(self):
        ...
        for gw in self.units(GATEWAY).ready.noqueue:
            if not self.units(STALKER).amount > self.units(IMMORTAL).amount:  # 粗略判断
                if self.can_afford(STALKER) and self.supply_left > 0:
                    await self.do(gw.train(STALKER))

        for sg in self.units().ready.noqueue:
